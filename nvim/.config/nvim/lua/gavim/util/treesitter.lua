...
function M.build(cb)
  M.ensure_treesitter_cli(function(_, err)
    ...
    if ok then
      ...
      GaVim.error(lines, { title = 'GaVim Treesitter' })
    end
  end)
end
...
function M.ensure_treesitter_cli(cb)
  ...
  mr.refresh(function()
    ...
    if not p:is_installed() then
      GaVim.info 'Installing `tree-sitter-cli` with `mason.nvim`...'
      p:install(
        nil,
        vim.schedule_wrap(function(success)
          if success then
            GaVim.info 'Installed `tree-sitter-cli` with `mason.nvim`.'
            cb(true)
          else

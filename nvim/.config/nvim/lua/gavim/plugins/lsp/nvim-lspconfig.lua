      ...
      servers = {
        ...
        ['*'] = {
          ...
          keys = {
            ...
            -- Commenting in favor of Snacks. Check snacks.lua for these keymaps
  ...
  config = vim.schedule_wrap(function(_, opts)
    ...
    for server, server_opts in pairs(opts.servers) do
      if type(server_opts) == 'table' and server_opts.keys then
        GaVim['lsp-keymaps'].set({ name = server ~= '*' and server or nil }, server_opts.keys)
      end
    end
    ...
    if opts.capabilities then
      vim.notify("lsp-config.opts.capabilities is deprecated. Use lsp-config.opts.servers['*'].capabilities instead", vim.log.levels.WARN)
